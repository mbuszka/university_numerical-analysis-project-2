...
function spline_cyclic_k(x, y; debug = false)
  n = size(x,1) - 1
  T = eltype(x)
  
  # Tworzymy macierz dla naszego układu równań
  a, b, g, h, f = build_matrix_k(x, y)
 
  # Rozwiązujemy układ równań
  k = solve_matrix(a, b, g)
  s = build_spline_k(x, y, f, h, k)
  s, k
end

function build_spline_k(x, y, f, h, k, debug = true)
  T = eltype(x)
  n = size(h, 1)
  w = Array{Array{T}}(n)
  w[1] = Array{T}(4)
